...
async def get_public_access_profile_id(dbi):
    ...
    return await _get_system_profile('public', dbi.get_public_profile)


async def get_authenticated_access_profile_id(dbi):
    """Get the authenticated access profile row ID from the cache or database."""
    return await _get_system_profile('authenticated', dbi.get_authenticated_profile)
...
async def _get_system_profile(key_str, get_profile_func):
