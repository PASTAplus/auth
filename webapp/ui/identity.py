        ...
        identity_list.append(
            {
                ...
                'avatar_url': util.avatar.get_profile_avatar_url(identity_row),
                'idp_logo_url': util.url.get_idp_logo_url(identity_row.idp_name),
                'common_name': identity_row.email,
                'profile': f'This profile ({token_profile_row.common_name})',
                'edi_id': token_profile_row.edi_id,
                'sort_key': 0,
            }
        )

    for linked_profile_row in await dbi.get_linked_profiles(token_profile_row):
        for identity_row in linked_profile_row.identities:
            identity_list.append(
                {
                    'idp_name': identity_row.idp_name,
                    'idp_uid': identity_row.idp_uid,
                    ...
                    'idp_logo_url': util.url.get_idp_logo_url(identity_row.idp_name),
                    'common_name': identity_row.email,
                    'profile': f'Linked profile ({linked_profile_row.common_name})',
                    'edi_id': linked_profile_row.edi_id,
                    'sort_key': 1,
                }
            )

    identity_list.sort(key=lambda x: (x['sort_key'], x['idp_name'].name, x['common_name']))
...
async def post_identity_unlink(
    ...
    await dbi.delete_identity(token_profile_row, db.models.profile.IdpName[idp_name_str], idp_uid)
