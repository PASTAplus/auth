...
async def get_ui_group_member(
    ...
    group_row = udb.get_group(token_profile_row, int(group_id))
...
async def post_group_new(
    ...
    udb.create_group(token_profile_row, name, description)
...
async def post_group_edit(
    ...
    name = form_data.get('name')
    description = form_data.get('description')
    udb.update_group(token_profile_row, group_id, name, description)
    return util.redirect.internal('/ui/group')
...
async def post_group_delete(
    ...
    form_data = await request.form()
    group_id = form_data.get('group-id')
    udb.delete_group(token_profile_row, group_id)
    return util.redirect.internal('/ui/group')
...
async def post_group_member_list(
    ...
    group_row = udb.get_group(token_profile_row, int(group_id))
    member_list = udb.get_group_member_list(token_profile_row, group_row.id)
...
async def post_group_member_add_remove(
    ...
    group_row = udb.get_group(token_profile_row, request_dict['group_id'])
    f = udb.add_group_member if is_add else udb.delete_group_member
    # noinspection PyArgumentList
    f(token_profile_row, group_row.id, request_dict['member_profile_id'])
