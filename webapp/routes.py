...
def login(idp):
    ...
    if idp == "pasta":
        ...
        github_client_id, _ = get_github_client_info(
            target=target, request_base_url=request.base_url
        )
        ...
        redirect_uri = f"{request.base_url}/callback/{target}"
        request_uri = (
            f"{authorization_endpoint}?client_id={orcid_client_id}"
...
def github_callback(target):
    ...
    github_client_id, github_client_secret = get_github_client_info(
        target=target, request_base_url=request.base_url
    )
...
def make_target_url(target: str, auth_token: str, cname: str) -> str:
    ...
    if target in (Config.PORTAL, Config.PORTAL_S, Config.PORTAL_D):
        ...
        url = f"https://{target}?token={_auth_token}&cname={_cname}"
    return url
